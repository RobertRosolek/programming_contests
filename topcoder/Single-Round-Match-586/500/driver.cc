...
vector <string>	queries;
		...
		__in >> __expected >> dynasties >> battles >> queries;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << dynasties << ", " << battles << ", " << queries << std::endl;
		std::cout.flush();

		__timer_start();

		History __object;
		string __actual = __object.verifyClaims(dynasties, battles, queries);
