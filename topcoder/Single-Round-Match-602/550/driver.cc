...
int	YC;
		...
		__in >> __expected >> N >> XS >> YS >> XA >> XB >> XC >> YA >> YB >> YC;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << XS << ", " << YS << ", " << XA << ", " << XB << ", " << XC << ", " << YA << ", " << YB << ", " << YC << std::endl;
		std::cout.flush();

		__timer_start();

		PilingRectsDiv1 __object;
		long long __actual = __object.getmax(N, XS, YS, XA, XB, XC, YA, YB, YC);
