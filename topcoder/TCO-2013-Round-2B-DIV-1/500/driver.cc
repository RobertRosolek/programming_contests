...
vector <string>	metro;
		...
		__in >> __expected >> taxi >> bus >> metro;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << taxi << ", " << bus << ", " << metro << std::endl;
		std::cout.flush();

		__timer_start();

		ScotlandYard __object;
		int __actual = __object.maxMoves(taxi, bus, metro);
