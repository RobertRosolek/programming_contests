...
int	B;
		...
		__in >> __expected >> N >> A >> B;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << A << ", " << B << std::endl;
		std::cout.flush();

		__timer_start();

		WellTimedSearch __object;
		double __actual = __object.getProbability(N, A, B);
